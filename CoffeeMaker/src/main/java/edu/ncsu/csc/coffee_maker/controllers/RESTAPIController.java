...
public class RESTAPIController {
    ...
    public ResponseEntity makeCoffee ( @PathVariable ( "id" ) final String id, @RequestBody final int amtPaid ) {
        ...
        try {
            ...
            if ( change == amtPaid && change != 0 ) {
                System.out.println( "change: " + amtPaid );
                final Inventory inventory = Application.getCoffeeMaker().getInventory();

                if ( inventory.enoughIngredients( recipe ) ) {
                    return new ResponseEntity<String>( "{\"result\":\"failureFunds\", \"change\":" + amtPaid + "}",
                            HttpStatus.EXPECTATION_FAILED );
                }
                else {
                    return new ResponseEntity<String>( "{\"result\":\"failureInventory\", \"change\":" + amtPaid + "}",
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        ...
        try {
            ...
            if ( !Application.getCoffeeMaker().getRecipeBook().addRecipe( recipe ) ) {
                ...
                throw new Exception( "Insufficient space in recipe book for recipes" );
            }
    ...
    /**
     * REST API endpoint to provide edit access to CoffeeMaker's recipes. This
     * will update the Recipe of the CoffeeMaker by updating the recipe with the
     *
     * @param recipe
     *            recipe containing name of recipe to edit
     * @return response to the request
     */
    @PutMapping ( BASE_PATH + "/recipes" )
    public ResponseEntity editRecipe ( @RequestBody @Valid final Recipe recipe ) {
        /*
         * final Recipe tmp =
         * Application.getCoffeeMaker().getRecipeBook().findRecipe(
         * recipe.getName() ); if ( tmp != null && recipe.getId().intValue() !=
         * tmp.getId().intValue() ) { return new ResponseEntity(
         * "Recipe with the name " + recipe.getName() + " already exists",
         * HttpStatus.CONFLICT ); }
         */
        try {
            final String result = Application.getCoffeeMaker().getRecipeBook().editRecipeByName( recipe );
            if ( result == null ) {
            ...
            // id does not match a recipe in the recipe book
            return new ResponseEntity( "Error accessing recipe book to edit recipe", HttpStatus.NOT_ACCEPTABLE );
        }
        catch ( final Exception e ) {
            return new ResponseEntity( "Insufficient space in recipe book for recipe " + recipe.getName(),
                    HttpStatus.INSUFFICIENT_STORAGE );
