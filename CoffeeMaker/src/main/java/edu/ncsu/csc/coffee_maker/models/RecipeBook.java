...
public class RecipeBook {
    ...
    /** Total number of recipes allowed in a recipe book at a time */
    public static final int RECIPE_CAPACITY = 3;
    ...
    public void updateRecipes () {
        ...
        recipesList = new Vector<Recipe>( RECIPE_CAPACITY );
    ...
    public synchronized boolean addRecipe ( final Recipe r ) {
        if ( recipesList.contains( r ) || recipesList.size() >= RECIPE_CAPACITY ) {
    ...
    /**
     * Returns the name of the recipe edited given a name within list and null
     * if the recipe does not exist.
     *
     * @param newRecipe
     *            edited recipe
     * @return name of edited recipe
     */
    public synchronized String editRecipeByName ( final Recipe newRecipe ) {
        int pos = -1;
        for ( int i = 0; i < recipesList.size(); i++ ) {
            if ( recipesList.get( i ).getName().equals( newRecipe.getName() ) ) {
                pos = i;
            }
        }
        return editRecipe( pos, newRecipe );
    }
}
